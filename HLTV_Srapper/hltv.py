...
def get_parsed_page(url, proxy_=None, auth_=None):
    ...
    got_req = requests.get(url, headers={"User-Agent": "Mozilla Firefox 105.0"}).text
...
class LastMaps:
    ...
    def add_all_params(self):
        time_start = time.time()
        for date, t1, t2, index in zip(self.df['date'][self.start_index:self.finish_index],
                                       self.df['url_team1'][self.start_index:self.finish_index],
                                       self.df['url_team2'][self.start_index:self.finish_index],
                                       range(len(self.df['date'][self.start_index:self.finish_index]))):
...
class Tour:
    ...
    def add_all_params(self):
        ...
        for event, index in zip(self.df['event_url'][self.start_index:self.finish_index],
                         range(len(self.df['event_url'][self.start_index:self.finish_index]))):
            self.MATCH_ID = index + self.start_index
            print(PREFIX + event)
            print('match id -', self.MATCH_ID, '; time spent -', time.time() - time_start)
            self.event = event
...
class PlStatInTeam:
    ...
    def _get_player_stat_(self, team_id, player):
        ...
        try:
            player_url = (self.df[f'players_url_{team_id}'][self.MATCH_ID])[player]
        except:
            player_url = (eval(self.df[f'players_url_{team_id}'][self.MATCH_ID])[player])
        if player_url:
...
class PlStatAll:
    ...
    def _get_player_stat_(self, team_id, player_id, params_1, params_2):
        ...
        player_url = (self.df[f'players_url_{team_id}'][self.MATCH_ID])[player_id]
        if player_url:
    ...
    def add_map_in_df(self, maps_name):
        ...
        for map_name in maps_name:
            ...
            if map_name:
                ...
                for team_id in range(1, 3):
                    for i in columns:
                        if i + map_name + f'_team{team_id}' not in self.df:
                ...
                for team_id in range(1, 3):
                    if check_null_in_line(team_id, columns, columns1, columns2):
                        url = MAP_STAT_PR + str(id_maps[map_name]) + '/' +\
                              (self.df[f'url_team{team_id}'][self.idx]).split('/team/')[-1] +\
                              f'?startDate={self.start_date}&endDate={self.finish_date}'
                        print(url)
        ...
        for map_url, map_name, score1_map, score2_map, counter \
            ...
            if map_name:
                ...
                for team_id in range(1, 3):
                    #if check_null_in_line(team_id, columns, columns1, columns2):
                    url = MAP_STAT_PR + str(id_maps[map_name]) + '/' +\
                          (self.df[f'url_team{team_id}'][self.MATCH_ID]).split('/team/')[-1] +\
                          f'?startDate={self.start_date}&endDate={self.finish_date}'
                    print(url)
                    self.STAT_PAGE = get_parsed_page(url, proxyDict, proxyAuth)
